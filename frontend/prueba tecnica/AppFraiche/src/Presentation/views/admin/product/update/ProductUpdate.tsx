...
export const AdminProductUpdateScreen = ({ navigation, route }: Props) => {
  ...
  const { user, name, description, responseMessage, loading, image1, image2, image3, price, errorMessage, stock, sku, sku_alt, purchase_department, promo_price, promo_quantity, tax, price3, setErrorMessage, setResponseMessage, onChange, takePhoto, pickImage, updateProduct } = useViewModel(product, category);
  const [modalVisible, setModalVisible] = useState(false);
  const [modalVisibleSide, setModalVisibleSise] = useState(false);
